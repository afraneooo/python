...
def salvar():
  if email.get() == '':
    ...
    c1.setNome(txtnome.get())
    c1.setEmail(txtemail.get())
    c1.setEndereco(txtendereco.get())
    c1.setNumero(txtnumero.get())
    c1.setEstado(cboestado.get())
    c1.setCep(txtcep.get())
    c1.setBairro(txtbairro.get())
    c1.setCidade(cbocidade.get())
    c1.setGenero(cbogenero.get())
    c1.setTelefone(txttelefone.get())
    c1.setCelular(txtcelular.get())
    if est_cv.get()=="Casado":
      c1.setEstado_civil(True)
    else:
      c1.setEstado_civil(False)
    c1.setEstuda(estuda.get())
